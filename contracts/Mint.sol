...
contract Mint is ERC721URIStorage {
    ...
    string[] firstWords = ["Iphone", "Shampoo", "Crazy", "Crypto", "World", "Pretty","Solidity","Difficult","Amazing","Developer", "Brilliant", "Pessimistic"];
    ...
    function pickFirstRandom(uint256 tokenId) public view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked("FIRST_WORD", Strings.toString(tokenId))));
        rand = rand % firstWords.length;
        return firstWords[rand];
    }

    function pickSecondRandom(uint256 tokenId) public view returns (string memory) {
        uint256 rand = random(string(abi.encodePacked("SECOND_WORD", Strings.toString(tokenId))));
        rand = rand % secondWords.length;
        return secondWords[rand];
    }

    function pickThirdRandom(uint256 tokenId) public view returns (string memory) {
    ...
    function mintYourNft() public {
        ...
        string memory second = pickSecondRandom(newId);
        string memory third = pickThirdRandom(newId);
